...
def process_video_thread(video_id):
	try:
		...
		handle_id = torrent_manager.add_torrent(movie_file.magnet_link, str(settings.DOWNLOAD_PATH))
		...
		with handle_lock:
			...
			downloaded_path = os.path.join(str(settings.DOWNLOAD_PATH), largest_file.path)
...
class MovieViewSet(viewsets.ViewSet):
	...
	def list(self, request):
		...
		try:
			...
			movies = [movie for movie in data["results"] if movie.get("poster_path")]
		...
		try:
			...
			if movie_file.download_status == "DOWNLOADING":
				...
				if handle:
					...
					num_pieces = torrent_info.num_pieces()
					downloaded_size = sum(1 for i in range(num_pieces) if handle.have_piece(i)) * piece_length
		...
		try:
			...
			if movie_file.download_status == "DOWNLOADING":
				...
				if movie_file.download_progress >= 5:
					...
					status_response["buffer_needed"] = 5 - movie_file.download_progress
